...
function initEvent() {
  ...
  $('body').on('click', '.alarmConfirmBtn', function() {
    var curHtml = '<div class="form-group mb5">' + '<label>告警是否准确：</label>' + '<label class="radio-inline p0 ml5"><input type="radio" name="IS_CORRECT" value="1" checked="checked">是</label>' + '<label class="radio-inline p0 ml5"><input type="radio" name="IS_CORRECT" value="0">否</label></div>';
    var $this = $(this),
  ...
  $('body').on('click', '.openRepeat', function() {
    ...
    var url = '/efacecloud/page/alarm/alarmFSCardList.html?OBJECT_ID=' + OBJECT_ID + '&BEGIN_TIME=' + bT + '&END_TIME=' + eT + '&timeControl=' + timeControl + '&KEYWORDS=' + KEYWORDS + '&DEVICE_IDS=' + DEVICE_IDS + '&AlGORITHM_ID=' + AlGORITHM_ID + '&DB_ID=' + DB_ID + '&SORT=' + SORT + '&IS_ESCAPE_HIT=' + IS_ESCAPE_HIT + '&THRESHOLD=' + THRESHOLD + '&alarmLevel=' + alarmLevel + '&alarmHandle=' + alarmHandle + '&isCatch=' + isCatch;
  ...
  $('body').on('click', '.searchBtn', function() {
    ...
    curSrc += "?imgSrc=" + imgurl + "&pageType=record" + '&buriedPointID=' + id;
  ...
  $("body").on("click", ".trajectory-search", function() {
    ...
    openWindowPopup('track', $(this).attr("url"), time, null, id);
  });
  //时间相似度排序
  ...
  $('body').on('click', '.openRepeatAll', function() {
    ...
    var url = '/efacecloud/page/alarm/alarmAllCardList.html?OBJECT_ID=' + OBJECT_ID + '&BEGIN_TIME=' + bT + '&END_TIME=' + eT + '&timeControl=' + timeControl + '&THRESHOLD=' + THRESHOLD + '&DEVICE_IDS=' + DEVICE_IDS + '&KEYWORDS=' + KEYWORDS + '&SORT=' + SORT + '&IS_ESCAPE_HIT=' + IS_ESCAPE_HIT + '&ALARM_TYPE=' + ALARM_TYPE + '&alarmLevel=' + alarmLevel + '&alarmHandle=' + alarmHandle + '&isCatch=' + isCatch + '&MULIT_ALGO_TYPE=' + allqueryParams.MULIT_ALGO_TYPE;
    UI.util.showCommonIframe('.frame-form-full', url);
  });
  //处置状态
  ...
  $("#freqAnalysisBtn").click(function() {
    ...
    UI.util.showCommonWindow("/efacecloud/page/alarm/freqAnalysis.html", "频次分析", 550, 400, function(data) {
      ...
      UI.util.showCommonIframe('.frame-form-full', '/efacecloud/page/alarm/alarmAnalysisList.html?BEGIN_TIME=' + beginTime + '&END_TIME=' + endTime + '&THRESHOLD=' + threshold + '&OBJECT_ID=' + objectId);
    });
  });
  //轨迹分析
  $("body").on("click", ".trajectory-searchAll", function() {
    ...
    if (isBlack) {
      ...
      var params = {
        src: "/efacecloud/page/technicalStation/tacticsFrame.html?pageType=track" + '&beginTime=' + time.bT + '&endTime=' + time.eT + '&noBackBtn=true&zjhm=' + $(this).attr('zjhm') + '&buriedPointID=' + id,
        title: '轨迹分析',
        width: $(top.window).width() * .95,
        height: $(top.window).height() * .9,
  ...
  $("body").on("click", ".verification-search", function() {
    ...
    var params = {
      src: src + query + '&buriedPointID=' + id,
      title: title,
      width: $(top.window).width() * .95,
      height: $(top.window).height() * .9,
      callback: function(obj) {}
...
$("body").on("click", ".structuredSearch", function() {
    ...
    UI.util.showCommonWindow("/datadefence/page/retrieval/structuredSearch.html?imgSrc=" + imgSrc, "结构化检索", 1200, 700, function(resp) {});
  })
...
function getAlarmDetailQuery($dom) {
  ...
  if (isfs == '1') {
    var name = $dom.attr("name"),
      ...
      time = $dom.attr("time");
    query = '?OBJECT_ID=' + OBJECT_ID + '&curTime=' + curTime + '&isFly=1&ALARM_ID=' + ALARM_ID + '&level=' + level + '&name=' + name + '&idCard=' + idCard + '&time=' + time + '&alarmTime=' + alarmTime + '&alarmImg=' + alarmImg;
  } else {
    query = '?OBJECT_ID=' + OBJECT_ID + '&curTime=' + curTime + '&ALARM_ID=' + ALARM_ID + '&level=' + level + '&alarmTime=' + alarmTime + '&alarmImg=' + alarmImg;
  }
  if (confirmStatus) {
...
function renderFillId() {
  var type = $('.tab-btn.actives').text().trim();
  if (type === '所有告警比中列表') {
    return {}
  } else if (type === '二次告警比中列表') {
    return {}
  }
}