...
var CONSTANTS = {
	...
	RLK: [
		{DB_NAME: "四川", DB_ID: "10051"},
		{DB_NAME: "重庆", DB_ID: "10050"},
		{DB_NAME: "上海", DB_ID: "10031"},
		{DB_NAME: "云南", DB_ID: "10053"},
		{DB_NAME: "贵州", DB_ID: "10052"},
		{DB_NAME: "630关注人员", DB_ID: "10096"},
		{DB_NAME: "北京", DB_ID: "10011"},
		{DB_NAME: "浙江", DB_ID: "10033"},
		{DB_NAME: "江苏", DB_ID: "10032"},
		{DB_NAME: "西藏自治区", DB_ID: "10054"},
		{DB_NAME: "外管办证", DB_ID: "10098"},
		{DB_NAME: "河北", DB_ID: "10013"},
		{DB_NAME: "福建", DB_ID: "10035"},
		{DB_NAME: "天津", DB_ID: "10012"},
		{DB_NAME: "安徽", DB_ID: "10034"},
		{DB_NAME: "内蒙古自治区", DB_ID: "10015"},
		{DB_NAME: "山东", DB_ID: "10037"},
		{DB_NAME: "山西", DB_ID: "10014"},
		{DB_NAME: "江西", DB_ID: "10036"},
		{DB_NAME: "甘肃", DB_ID: "10062"},
		{DB_NAME: "陕西", DB_ID: "10061"},
		{DB_NAME: "湖北", DB_ID: "10042"},
		{DB_NAME: "宁夏回族自治区", DB_ID: "10064"},
		{DB_NAME: "河南", DB_ID: "10041"},
		{DB_NAME: "青海", DB_ID: "10063"},
		{DB_NAME: "吉林", DB_ID: "10022"},
		{DB_NAME: "广东", DB_ID: "10044"},
		{DB_NAME: "辽宁", DB_ID: "10021"},
		{DB_NAME: "湖南", DB_ID: "10043"},
		{DB_NAME: "新疆维吾尔自治区", DB_ID: "10065"},
		{DB_NAME: "海南", DB_ID: "10046"},
		{DB_NAME: "黑龙江", DB_ID: "10023"},
		{DB_NAME: "广西壮族自治区", DB_ID: "10045"},
		{DB_NAME: "重点人员", DB_ID: "100104"},
		{DB_NAME: "在逃人员", DB_ID: "100105"}
	],
//     RLK:[ //人脸库
//         {DB_ID: '-1',DB_NAME:'全部'},
...
function initRlk(){
	UI.control.remoteCall('face/common/feishiAlgoLib', {}, function(resp) {
		var data = JSON.parse(JSON.stringify(resp.DATA).replace(/id/g,"DB_ID").replace(/name/g,"DB_NAME"));
...
//					seriesImg.push($(this).find('img').eq(1).attr('src'));
		...
		var options = {
			...
			// 	initWaterMarkToZoom();
			// },
			// fnBPrevCallback: function(i){
			// 	initWaterMarkToZoom();
			// },
			// fnBNextCallback: function(i){
			// 	initWaterMarkToZoom();
			// },
...
function initWaterMarkToZoom() {
	setTimeout(function(){
		top.$(".zoomableContainer").each(function(index,item){
			$(item).find('.water-mark').remove();
			$(item).append("<div class='water-mark' style='" + top.customization.watermarkData + "'></div>");
		});
	},100)
}
...
function initDbTree(options){
	...
	$.each($addressWrap,function(i,n){
		var areaHtml = '<div class="tree-wrap dropdown">'+
			'<div class="modular-head dropdown-list dropdown-toggle" data-toggle="dropdown">'+
				'<span class="selectedDb"> - 请选择 - </span>'+
				'<input type="hidden" id="registerDb" name="'+selectArr[i]+'"  value="">'+
				'<span class="icon-arrow-down7 tc fr dropdown-right-icon"></span>'+
			'</div>'+
			'<div class="dropdown-menu">'+
				'<div class="db-list">'+
					'<div class="tab-pane active">'+
						'<dl class="attr-list">'+
							'<dd class="attr-list-body mCustomScrollbar" id="'+addressId[i]+'">'+
							'</dd>'+
						'</dl>'+
					'</div>'+
					'<div class="attr-bar"><a class="btn btn-i-t btn-i-l attrSureBtn mr10">确定</a><a class="btn btn-i-l btn-i-t attrCancelBtn">取消</a></div>'+
				'</div>'+
			'</div>'+
		'</div>';
		$("#"+n).append(areaHtml);
 ...
 * @author：lwb
 */
function picSuccFunction(data, status, callback) {
	...
	if (data && !data.error) {
		...
		$('[foruploadimg*="'+ fileElementId +'"]').siblings('.pciImageIcon').removeClass('pci-image-display');
		UI.util.hideLoadingPanel();
		return true;
	} else {
 ...
 * @Author lzh
 */
function slideFn(src,menuId) {
	var data = [];
    if(src){
        UI.control.remoteCall(src,{MENUID:menuId},function (resp) {
            if(resp.data.length>0){
                for(var i=0;i<resp.data.length;i++){
                    $("#arithmetic").append(tmpl('ALGTemplate',resp.data[i]));
    ...
    $("body").on('click',".arithmetic-tools span",function (e) {//点文字选择或者取消
        ...
        if($tools.hasClass('on')){
            ...
            if($tools.hasClass('all')){
                $(".arithmetic-tools:not(:eq(0))").removeClass('on').show().siblings().hide();
            }else {
                $(".arithmetic-tools").eq(0).removeClass('on').show().siblings().hide();
            }
        }
        ALGO_LIST = getInputVal();
        e.stopPropagation();
    ...
    function getInputVal() {
        ...
        if( $(".arithmetic-tools.on").hasClass('all') ){
        	algoList = getAllAlgorithm($(".arithmetic-tools:not('.all')"));
        }else{
        	algoList = getAllAlgorithm($(".arithmetic-tools.on"));
        }
    ...
    function getAllAlgorithm(elms){
    	...
    	elms.each(function (i) {
			...
			$.each(data,function(i,item){
				...
				if(item.ALGORITHM_ID == $this.attr("algo_type")){
					var obj = {
							ALGO_TYPE:$input.attr("algo_type"),
							THRESHOLD: Math.round($input.val()/item.SCORE_RATE)
						};
					algoList.push(obj);
				}
			});
 ...
 * @author lwb
 * */
function openWindowPopup( type, imgUrl, time){
	var src = '',
		title='';
	switch(type){
		case 'track':
			...
			if(time && time.bT && time.eT){
				src = matcher("/efacecloud/page/technicalStation/tacticsFrame.html/" + top.projectID).url + "?pageType=track&imgUrl=" + imgUrl +'&beginTime='+time.bT+'&endTime='+time.eT+'&noBackBtn=true';
			}else{
				src = matcher("/efacecloud/page/technicalStation/tacticsFrame.html/" + top.projectID).url + "?pageType=track&imgUrl=" + imgUrl+'&noBackBtn=true';
			}
			break;
		case 'identity':
			title = '身份核查';
			src = matcher("/efacecloud/page/technicalStation/verification.html/" + top.projectID).url + '?imgUrl=' + imgUrl;
			break;

		case 'faceCapture':
			title = '路人检索';
			src = matcher("/efacecloud/page/library/faceCaptureList.html/" + top.projectID).url + '?imgSrc=' + imgUrl;
			break;
		case 'faceStore':
			title = '人员档案库';
			src = matcher("/efacestore/page/library/personnelFileMagList.html/" + top.projectID).url + '?imgSrc=' + imgUrl;
			break;
	}
...
function initDeviceListData(resp,index){
    ...
    if(dName.length>0&&resp.deviceName.length>0){
		...
		$('#latitude'+curIndex).val(resp.latitude || '');
		$('#longitude'+curIndex).val(resp.longitude || '');
...
function initWaterMark($ele){
    ...
    $('.pager-content').watermark({
        texts : textsArr, //水印文字
		// textColor : "#12459d", //文字颜色
		textColor: 'rgba(82, 119, 183, .2)',
        textFont : '16px 宋体', //字体
        width : 150, //水印文字的水平间距
...
//获取默认主算法
function getAlgoListMain(){
	UI.control.remoteCall("face/common/getFaceAlgoType",{MENUID:"EFACE_faceDispatchedMain"},function(resp){
		if(resp && resp.data){
			scoreRate = resp.data[0].SCORE_RATE;
		}
	});
}
...
/**
 * @author: dorsey
 * @description: 新版时间组件调用
 * @version: 2019-05-28
 * 
 *  */
function datePickerInit (options) {
	
	//时分秒年月日范围，max设置最大可选时间不能超过当下，min值

	var defaultOption = {
		// max: new Date().format('yyyy-MM-dd HH:mm:ss'),
		isRange: true,
		hide: function () {
			options.callback && 'function' === typeof options.callback && options.callback();
		}
	}

	$.extend(defaultOption, options.otherParams ? options.otherParams : {});
	
	options.$dom.datePicker(defaultOption);
	// var timeFormat = 'underfined' === typeof options.timeFormat ? 'yyyy-MM-dd hh:mm:ss' : options.timeFormat;

	var timeFormat = 'yyyy-MM-dd HH:mm:ss';

	var timeConfig = {

		"jt": UI.util.getDateTime("today", timeFormat),			//	今天

		"zt": UI.util.getDateTime("yesterday", timeFormat),	//	昨天

		"bz": UI.util.getDateTime("thisWeek", timeFormat),	//	本周

		"by": UI.util.getDateTime("thisMonth", timeFormat),	//	本月

		"nW": UI.util.getDateTime("nearWeek", timeFormat),	//	近7天

		"nM": UI.util.getDateTime("nearMonth", timeFormat),	//	近30天

		"bN": UI.util.getDateTime("thisYear", timeFormat)	//	今年
	}

	options.$dom.parent().on('click', '.tagsTime', function () {

		$(this).addClass('active').siblings('.tagsTime').removeClass('active');

		if("zdy" !== $(this).attr('time-control')) {

			$('.c-datepicker-picker').css('display', 'none');

			options.$dom.addClass('hide');

			options.$dom.find('#beginTime').val(timeConfig[$(this).attr('time-control')].bT);

			options.$dom.find('#endTime').val(timeConfig[$(this).attr('time-control')].eT);
	
			options.callback && 'function' === typeof options.callback && options.callback(timeConfig[$(this).attr('time-control')]);
			
		}else{
			$('.c-datepicker-picker').css('display', 'none');

			options.$dom.removeClass('hide');
		}
...
function getCaseInfo (IDENTITY_ID) {

	var res = false;
	UI.control.remoteCall("personal/details/queryCaseInformation",{IDENTITY_ID:IDENTITY_ID},function(resp){

		if(resp.CODE==0){

			if(resp.personDetailsListdata.length > 0){
				
				res = resp.personDetailsListdata;
				
			}else{
				UI.util.alert("该人员没有案情信息，无需打标签");
			}
		}else{
			UI.util.alert(resp.MESSAGE,"warn");
		}
