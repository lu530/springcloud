...
var dateType = UI.util.getUrlParam("dateType") || '';
...
function initEvent() {
    ...
    $('#sureBlackBtn').click(function(){
    	...
    	if ($('#filterImg')[0].src.slice(-12) != "noPhoto2.png" || $(".barItem.active").attr("attr-type") === "source") {
        	...
        	var checkDataArr = UI.control.getControlById('faceCollectionList').getListviewCheckData();
        }
    	...
    	UI.util.confirm('已勾选的都确认为外籍人？',function(){
    		var infoIdArr = [];
    		for(var i=0;i<checkDataArr.length;i++){
    			infoIdArr.push(checkDataArr[i].INFO_ID);
    		}
    		var opts = {
    				isBlack:true,
    				...
    				okCallback:function(){
    					for(var i=0;i<checkDataArr.length;i++){
    						$('.list-node-wrap .switchBtn[infoid="'+checkDataArr[i].INFO_ID+'"],.table .switchBtn[infoid="'+checkDataArr[i].INFO_ID+'"]').addClass('active');
    		    		}
    				}
    		}
    ...
    //算法确认统计
    $('#sureStaticsBtn').click(function(){
    	getParams();
    	var paramsObj = {};
        $.extend(true,paramsObj,queryParams);
        top.globalCache.faceCaptureParams = paramsObj;
        if(queryParams.ALGO_LIST == ''){
        	top.globalCache.faceCaptureParams.ALGO_LIST = JSON.stringify(getAlgoList());
        }
    	UI.util.showCommonWindow('/efacecloud/page/library/faceCaptureBlackAlgoList.html', '算法确认统计', $(top.window).width()*.95, $(top.window).height()*.9, function(data) {});
    })
	...
	$('body').on('click','.switchBtn',function(){
		...
		/*}else{
			...
			if($this.hasClass('active')){
				$this.removeClass('active');
				$('.algo-type-list').addClass('hide');
				
				if(queryParams.PIC){
					$('.page-con-sort').removeClass('hide');
					$('.page-con').addClass('hide');
				}else{
					$('.action-btn-group .active[attr-type]').click();
				}
			}else{
				$this.addClass('active');
				$('.algo-type-list,.page-con').removeClass('hide');
				$('.page-con-sort,.page-con .cardBox,.page-con .tableBox').addClass('hide');
			}
...
function getParams(){
	...
	var curRaceType = $('#race .active').attr('value');
    if ($(".arithmetic-tools.on").hasClass('all')) {
        queryParams.ALGOALL = 1;
        
        if(!curRaceType){
        	queryParams.ALGO_LIST = '';
        }
                    ...
                    if (sortAlgorithmObj[i].ALGORITHM_LIST.length > 0) {
                        for (key in sortAlgorithmObj[i]) {
                            if (key == 'ALGORITHM_ANME') {
								if(sortAlgorithmObj[i].ALGORITHM_ANME == "华云"){
									keyArr.unshift(sortAlgorithmObj[i][key]);
								}else{
                            ...
                            if (key == 'ALGORITHM_LIST') {
								...
								if(sortAlgorithmObj[i].ALGORITHM_ANME == "华云"){
									sortAlgorithmArr.unshift(sortAlgorithmObj[i][key]);
								}else{
...
//同步是否外籍人
function getSynBlackList(){
	global.blackList =[]; 
	$.each($('.switchBtn.active:visible'),function(i,n){
		global.blackList.push($(n).attr('infoid'));
	});
}
function setSynBlackList(){
	if(global.blackList){
		$('.switchBtn:visible').removeClass('active');
		for(var i=0;i<global.blackList.length;i++){
			$('.switchBtn[infoid="'+global.blackList[i]+'"]').addClass('active');
		}
	}
}
...
function renderAlgo(algoStr){
	var algoArr = algoStr.split(' '),
		algoList = global.algoNumSnList,
		algoObj = {},
		html = '';
	
	for(var i=0;i<algoList.length;i++){
		var index = algoArr.indexOf(algoList[i].ALGORITHM_ID);
		if(index>-1){
			html += '<span class="alg-tag-item active"><span>'+algoList[i].ALGORITHM_NAME+'</span></span>';
		}else{
...
function defaultSelectAllDevice(){
	var treeService ='user/structure/num/getTree';
	var listService = 'cp/device/getDeviceList';//获取设备列表服务
	
	var orgCode = "44";
	UI.control.remoteCall(treeService, {}, function(resp){
		
		if(resp && resp.data){
			UI.util.debug("---------------" + resp.data[0].ORG_CODE);
			orgCode = resp.data[0].ORG_CODE;
		}
	});
	
	UI.control.remoteCall(listService, {CASCADE:1,DEVICE_TYPE:"194",ORG_CODE:orgCode}, function(resp){
		
		var deviceNameArr = [], deviceIdArr = [], deviceIdIntArr = [], orgCodeArr = [];
		
		$.each(resp.data,function(i, item){
			deviceNameArr.push(item.DEVICE_NAME);
			deviceIdArr.push(item.DEVICE_ID );
			deviceIdIntArr.push(item.DEVICE_ID_INT);
			orgCodeArr.push(item.ORG_CODE);
		});
		
		var deviceId = deviceIdArr.join(",");
		var deviceName = deviceNameArr.join(",");
		var deviceIdInt = deviceIdIntArr.join(",");
		var orgCode = orgCodeArr.join(",");
		
		$('#deviceNames').html(deviceName);
		$('#deviceNames').attr('title', deviceName);
		$('#deviceNames').attr('orgcode', orgCode);
		$('#orgCode').val(deviceId);
		$('#orgCodeInt').val(deviceIdInt);
