...
public class FaceDispatchedAlarmDao {
	...
	public List<Map<String, Object>> getAlarmDetail(String alarmId) {
		String detailSql = "select p.PERSON_ID,vfa.OBJECT_ID, vfa.ALARM_TIME, vfa.SCORE, vfa.DB_ID,vfa.DB_NAME, vfa.OBJECT_EXTEND_INFO, d.ORG_CODE, d.NAME ORG_NAME,"
				...
				+ " u.USER_NAME, dp.DEPT_NAME, edp.CASE_ID from VPLUS_SURVEILLANCE_ALARM vfa "
	...
	public Integer countAlarm(Map<String, Object> parameters) {
		...
		if (!StringUtil.isNull(confirmStatus)) {
			if (isTableExist("eface_police_task_dispatch") && isTableExist("eface_police_task_record")
					&& isTableExist("eface_police_task_remark")) {
		...
		if (!"admin".equals(userCode)) {
			...
			if ("1".equals(IS_PERSONEL_CONTROL_ELABORATION)) {
				...
				sb.append(" and (d.TAG_CODE = '01' ");
				sb.append(" or (d.TAG_CODE = '02' and (EXISTS ( "
						+ "SELECT 1 FROM SYS_USERFUNC uf, SYS_FUNLIST f WHERE ? = uf.USER_CODE AND uf.ORG_CODE = f.FUNID "
						+ "AND f.MENUID = ? and u.DEPT_CODE like ?) ");
				sb.append(" or (exists(select 1 from V_VPLUS_DEVICE_INFO vdi "
						+ "where vdi.DEVICE_ID = vfa.DEVICE_ID and vdi.ORG_CODE = ?) ");
				sb.append(") or vdp.CREATOR = ? or exists ( "
						+ "select 1 from EFACE_DISPATCHED_PERSON_AUTHORITY au where au.PERSON_ID = vdp.PERSON_ID and au.USER_CODE = ?))) ");

				sb.append(" or (d.TAG_CODE = '03' and (EXISTS ( "
						+ "select 1 from EFACE_DISPATCHED_PERSON_AUTHORITY au where au.PERSON_ID = vdp.PERSON_ID and au.USER_CODE = ?) "
						+ "or vdp.CREATOR = ? ");
				sb.append(" or (EXISTS (SELECT 1 FROM SYS_USERFUNC uf, SYS_FUNLIST f WHERE ? = uf.USER_CODE "
						+ "AND uf.ORG_CODE = f.FUNID AND f.MENUID = ? and u.DEPT_CODE like ?)))))");

				list.add(userCode);
				list.add(Constants.DISPATCHED_PERSON_PERMISSION_MENUID);
				list.add(deptCode + "%");
				list.add(StringUtil.toString(parameters.get("CIVIL_CODE")));
				list.add(userCode);
				list.add(userCode);
				list.add(userCode);
				list.add(userCode);
