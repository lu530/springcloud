 ...
 */
public class InternetCafesFaceEsProvider extends IndexSearchProvider {
	
	public InternetCafesFaceEsProvider() {
		super(EAP.bigdata);
	}
	...
	private List<Map<String, Object>> render(List<Map<String, Object>> resultSet) throws Exception {
		...
		for (Map<String, Object> map : resultSet) {
			...
			if (!StringUtil.isNull(algoTyoeStr)) {
				map.put("ALGORITHM_NAME", ModuleUtil.getAlgorithmById(Integer.valueOf(algoTyoeStr)).get("ALGORITHM_NAME"));
			}
			list.add(map);
		}
		return list;
	...
	public void prepare(RequestContext context, Query query) throws Exception {
		...
		if (!StringUtil.isEmpty(keyword)) {
			...
			List<Map<String, Object>> deviceList = resDao.getDeviceIdByTypeAndKeyword(StringUtil.toString(Constants.DEVICE_TYPE_FACE), keyword);
