...
public class FaceFavoriteFileProvider extends ExportGridDataProvider
{
	@Override
	...
	{
		String sql = "select FILE_ID, DISPATCHED_DB_NAME, DEVICE_ID, DEVICE_NAME, NAME, SEX, IDENTITY_TYPE, IDENTITY_ID, "
	...
	protected void prepare(RequestContext context) 
	{
		...
		if (!StringUtil.isNull(personTag)) {
			addOptionalStatement(" and FILE_ID in (SELECT DISTINCT REL_ID FROM EFACE_TAG_REL WHERE TAG_CODE in"+ SqlUtil.getSqlInParams(personTag)+")");
	...
	@BeanService(id="exportFaceSearch",description="我的收藏结果导出")
		...
		if ("1".equals(fileType)) {
			...
			try {
		    	for (Map<String,Object> person : excelDataList){
		            ...
		            DeviceEntity faceDevice = (DeviceEntity)EAP.metadata.getDictModel(DictType.D_FACE, StringUtil.toString(person.get("DEVICE_ID")), DeviceEntity.class);
