...
//            				+ " where rem.REMARK_KEY is not null GROUP BY d.REL_ID )" );
                ...
                Log.faceSearchLog.info("告警查询-确认-表不存在。。。。");
            }
...
//        }
        boolean tablesExist = isAlarmTablesExist();
        if (!tablesExist) {
            // 是否告警确认 0:否 1：是
            if (!StringUtil.isNull(confirmStatus)) {
                this.addOptionalStatement(" and vfa.CONFIRM_STATUS = ? ");
    ...
    private boolean isAlarmTablesExist() {
        if (dao.isTableExist("EFACE_POLICE_TASK_DISPATCH") && dao.isTableExist("EFACE_POLICE_TASK_RECORD") && dao.isTableExist("EFACE_POLICE_TASK_REMARK")) {
            return true;
        }
        return false;
    }
